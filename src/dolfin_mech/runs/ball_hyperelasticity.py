...
"""Defines the Ball_Hyperelasticity driver.

Benchmark simulation function that
orchestrates the automated generation of a spherical mesh and the solution of
a finite strain hyperelastic problem under radial expansion, supporting both
compressible and mixed incompressible formulations.
"""
...
def Ball_Hyperelasticity(
	...
	**Workflow:**
	...
	mesh, boundaries_mf, S_id = Ball_Mesh(params=mesh_params)
	...
	if (
		...
		surface_nodes_coords = [
			...
			if dolfin.near(
				(node_coords[0] - X0) ** 2 + (node_coords[1] - Y0) ** 2 + (node_coords[2] - Z0) ** 2, R**2, eps=1e-3
			)
		]
		...
		for X in surface_nodes_coords:
			...
			X_sd = dolfin.CompiledSubDomain(
				"near(x[0], x0) && near(x[1], y0) && near(x[2], z0)", x0=X[0], y0=X[1], z0=X[2]
			)
	...
	problem.add_point_displacement_qoi(name="U", coordinates=[X0 + R, Y0, Z0], component=0)
