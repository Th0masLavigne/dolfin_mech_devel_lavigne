...
"""Defines the HeartSlice_Hyperelasticity driver.

Specialized biomechanical
benchmark that simulates the nonlinear deformation of a ventricular
cross-section. It supports kinematic "twisting" through prescribed radial and
tangential displacements, as well as physiological inflation via endocardial
pressure loading on an annular domain.
"""
...
def HeartSlice_Hyperelasticity(
	...
	**Loading Scenarios:**
	...
	mesh, boundaries_mf, Si_id, Se_id, points_mf, x1_sd, x2_sd, x3_sd, x4_sd = HeartSlice_Mesh(params=mesh_params)
	...
	problem = problems.Hyperelasticity(
		mesh=mesh,
	...
	Deltat = step_params.get("Deltat", 1.0)
	dt_ini = step_params.get("dt_ini", 1.0)
	dt_min = step_params.get("dt_min", 1.0)
	...
	if (
		load_type == "disp"
	):  # MG20220813: It would be possible to impose the spatially varying displacement directly through an expression, but this would need to be implemented within Constraint, e.g. with a TimeVaryingExpression.
		internal_nodes_coords = [
			node_coords
			for node_coords in mesh.coordinates()
			if dolfin.near((node_coords[0] - X0) ** 2 + (node_coords[1] - Y0) ** 2, Ri**2, eps=1e-3)
		]
		...
		external_nodes_coords = [
			node_coords
			for node_coords in mesh.coordinates()
			if dolfin.near((node_coords[0] - X0) ** 2 + (node_coords[1] - Y0) ** 2, Re**2, eps=1e-3)
		]
		...
		problem.add_constraint(V=problem.displacement_subsol.fs.sub(1), sub_domain=x1_sd, val=0.0, method="pointwise")
		problem.add_constraint(V=problem.displacement_subsol.fs.sub(0), sub_domain=x2_sd, val=0.0, method="pointwise")
		problem.add_constraint(V=problem.displacement_subsol.fs.sub(1), sub_domain=x3_sd, val=0.0, method="pointwise")
		problem.add_constraint(V=problem.displacement_subsol.fs.sub(0), sub_domain=x4_sd, val=0.0, method="pointwise")
		p = load_params.get("p", 0.1)
		problem.add_surface_pressure_loading_operator(measure=problem.dS(Si_id), P_ini=0, P_fin=p, k_step=k_step)

	################################################# Quantities of Interest ###

	problem.add_point_displacement_qoi(name="ui", coordinates=[X0 + Ri, Y0], component=0)

	problem.add_point_position_qoi(name="ri", coordinates=[X0 + Ri, Y0], component=0)
