...
"""Defines the run_Disc_Mesh utility.

Automates the generation of a 2D
triangular disc mesh using Gmsh and provides pre-configured FEniCS
MeshFunctions for boundary and cardinal point identification.
"""
...
def Disc_Mesh(params: dict = {}):
	r"""Generates a 2D disc mesh using Gmsh and prepares boundary/point markers.
	...
	**Point Marking Strategy:**
	...
	S_sd = dolfin.AutoSubDomain(
		lambda x, on_boundary: on_boundary and dolfin.near((x[0] - X0) ** 2 + (x[1] - Y0) ** 2, R**2, eps=1e-3)
	)

	S_id = 1
	S_sd.mark(boundaries_mf, S_id)
