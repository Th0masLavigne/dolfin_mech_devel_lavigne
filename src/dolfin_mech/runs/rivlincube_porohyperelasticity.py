...
"""Defines the RivlinCube_PoroHyperelasticity driver.

High-level manager for
biphasic finite strain simulations. It orchestrates forward and inverse poro-
hyperelastic problems on structured domains, coupling solid skeleton mechanics
with fluid volume fraction evolution and pressure-balancing gravity loads for
...
def RivlinCube_PoroHyperelasticity(
	...
	r"""Runs the Rivlin Cube benchmark for Poro-Hyperelasticity (Forward or Inverse).
	...
	**Porosity Initialization (`porosity_params`):**
	...
	if cube_params.get("generic_zones", False):
		if len(mat_params) > 1:
			...
			for mat_id in range(number_zones - 1, -1, -1):
				subdomain_lst.append(
					dolfin.CompiledSubDomain(
						"x[1] <= y1 - tol", y1=ymax - length_zone * (number_zones - 1 - mat_id), tol=tol
					)
				)
	...
	if porosity_type == "constant":
		...
		porosity_mf = dolfin.MeshFunction("double", mesh, porosity_filename)
		porosity_expr = dolfin.CompiledExpression(
			getattr(dolfin.compile_cpp_code(core.get_ExprMeshFunction_cpp_pybind()), "MeshExpr")(),
			mf=porosity_mf,
			degree=0,
		)
		...
		if porosity_type == "mesh_function_constant":
			porosity_mf = dolfin.MeshFunction(value_type="double", mesh=mesh, dim=dim, value=porosity_val)
		...
		if porosity_type == "function_constant":
			...
			with open(porosity_filename, "w") as file:
				...
				file.write("</dolfin>\n")
				file.close()
			porosity_fun = dolfin.Function(porosity_fs, porosity_filename)
	...
	if inverse:
		problem = problems.InversePoroHyperelasticity(
			mesh=mesh,
		...
		problem = problems.PoroHyperelasticity(
			mesh=mesh,
			define_facet_normals=1,
			boundaries_mf=boundaries_mf,
			domains_mf=domains_mf,
	...
	if load_type == "internal":
		pf_lst = load_params.get(
			"pf_lst", [(k_step + 1) * load_params.get("pf", +0.5) / n_steps for k_step in range(n_steps)]
		)
	elif load_type in ("external0", "external"):
		P_lst = load_params.get(
			"P_lst", [(k_step + 1) * load_params.get("P", -0.5) / n_steps for k_step in range(n_steps)]
		)
	elif load_type in ("p_boundary_condition0", "p_boundary_condition"):
		f_lst = load_params.get(
			"f_lst", [(k_step + 1) * load_params.get("f", 1e4) / n_steps for k_step in range(n_steps)]
		)
	...
	for k_step in range(n_steps):
		...
		if load_type == "internal":
			...
			problem.add_pf_operator(measure=problem.dV, pf_ini=pf_old, pf_fin=pf, k_step=k_step)
		elif load_type == "external":
			problem.add_pf_operator(measure=problem.dV, pf_ini=0.0, pf_fin=0.0, k_step=k_step)
			P = P_lst[k_step]
			...
			problem.add_pressure_balancing_gravity_loading_operator(
				...
				P0_ini=P0_old,
				P0_fin=P0,
				breathing_constant=load_params.get("H", 0.0),
				k_step=k_step,
			)
		...
		if not inertia_params["applied"]:
			...
			problem.add_inertia_operator(measure=problem.dV, rho_val=rho_val, k_step=k_step)
	...
	if plot_curves:
		...
		for comp in ["skel", "bulk", "tot"]:
			...
			if dim == 3:
				all_stresses += ["s_" + comp + "_ZZ"]
			all_stresses += ["s_" + comp + "_XY"]
			if dim == 3:
				all_stresses += ["s_" + comp + "_YZ", "s_" + comp + "_ZX"]
