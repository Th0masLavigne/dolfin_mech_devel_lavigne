...
"""Defines the run_RivlinCube_Elasticity driver.

Comprehensive benchmark for
linearized and finite strain elasticity. It provides a modular environment to
validate constitutive models under diverse loading conditions—including
displacement, traction, and follower pressure—while supporting multi-material
heterogeneity and stable mixed formulations for incompressibility.
"""
	...
	**Features:**
	...
	if load_type == "disp":
		...
		problem.add_volume_force0_loading_operator(
			measure=problem.dV, F_ini=[0.0] * dim, F_fin=[f] + [0.0] * (dim - 1), k_step=k_step
		)
	elif load_type == "surf":
		f = load_params.get("f", 1.0)
		problem.add_surface_force0_loading_operator(
			measure=problem.dS(xmax_id), F_ini=[0.0] * dim, F_fin=[f] + [0.0] * (dim - 1), k_step=k_step
		)
	elif load_type == "pres":
		p = load_params.get("p", -1.0)
		problem.add_surface_pressure0_loading_operator(measure=problem.dS(xmax_id), P_ini=0, P_fin=p, k_step=k_step)
	elif load_type == "pgra":
		X0 = load_params.get("X0", [0.5] * dim)
		N0 = load_params.get("N0", [1.0] + [0.0] * (dim - 1))
		P0 = load_params.get("P0", -1.0)
		DP = load_params.get("DP", -0.5)
		problem.add_surface_pressure_gradient0_loading_operator(
			measure=problem.dS(), X0_val=X0, N0_val=N0, P0_ini=0.0, P0_fin=P0, DP_ini=0.0, DP_fin=DP, k_step=k_step
		)
	elif load_type == "tens":
		gamma = load_params.get("gamma", 0.01)
		problem.add_surface_tension0_loading_operator(measure=problem.dS, gamma_ini=0.0, gamma_fin=gamma, k_step=k_step)
